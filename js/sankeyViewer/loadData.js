...
function checkCryptoSupport() {
  if (!window.crypto || !window.crypto.subtle) {
    throw new Error('Web Crypto API niet beschikbaar. De site moet worden geladen via HTTPS of localhost.');
  }
}
...
async function decryptZipFile(passphrase) {
  try {
    ...
    // Check if using embedded data source
    if (typeof dataSource !== 'undefined' && dataSource === 'embedded') {
      // Use embedded data from window object
      if (typeof window.EMBEDDED_ENCRYPTED_DATA === 'undefined') {
        throw new Error('Embedded encrypted data not found');
      }
      encryptedPayload = window.EMBEDDED_ENCRYPTED_DATA;
    } else {
      // Fetch the encrypted file from URL
    ...
    if (error.message && error.message.includes('Web Crypto API')) {
      throw error; // Re-throw crypto API errors as-is
    ...
    } else if (error.message && error.message.includes('crypto.subtle')) {
      throw new Error('Web Crypto API niet beschikbaar. Gebruik HTTPS of localhost.');
    } else {
...
let XLSXurl = 'data/data_sankeydiagram_regionalviewer.xlsm'

let sankeyConfigs = []

function initTool () {
  // Don't initialize if on a phone device
  if (typeof isPhoneDevice !== 'undefined' && isPhoneDevice) {
    console.log('Viewer disabled on phone devices');
    return;
  }
  ...
  } else if (dataSource == 'file' || dataSource == 'embedded') {
    console.log(dataSource === 'embedded' ? 'EMBEDDED' : 'FILE')
...
// }, 1000)
 ...
 // Only create modern UI interface when dataSource is 'file' or 'embedded'
 if (typeof dataSource !== 'undefined' && (dataSource === 'file' || dataSource === 'embedded')) {
   
   // Don't initialize if on a phone device
   if (typeof isPhoneDevice !== 'undefined' && isPhoneDevice) {
     console.log('Encrypted data UI disabled on phone devices');
   } else {
     // Check crypto support early
     if (!window.crypto || !window.crypto.subtle) {
       const statusMessage = document.getElementById('statusMessage');
       if (statusMessage) {
         statusMessage.innerHTML = `
           <div style="color: #dc2626; padding: 20px; text-align: left; line-height: 1.6;">
             <strong style="font-size: 18px;">⚠️ Web Crypto API niet beschikbaar</strong><br><br>
              De viewer vereist een beveiligde verbinding. 
           </div>
         `;
       }
       console.error('Web Crypto API is not available. The site must be served over HTTPS or localhost.');
       throw new Error('Web Crypto API niet beschikbaar');
     }
     
     // Function to handle decryption and data loading
   async function handleDecryptionAndLoading(passphrase) {
     try {
    ...
    // NOTE: Waterfall diagram code removed - not applicable to this Sankey viewer
    // Pass CSV data to cijferbasis module if available (commented out - not used in this viewer)
    ...
    // GENERATE SANKEY
     ...
     sankeyConfigs.forEach(element => {
      ...
      // ID validation disabled for PBL public version
      // if (settings[0].projectID != projectID || settings[0].versionID != versionID || settings[0].productID != productID) {
      //   console.log('ERROR - ID MISMATCH')
      //   const loadFileDialog = document.getElementById('loadFileDialog')
      //   // Set the inner HTML with the desired text and link
   ...
   // Check if password is embedded - if so, auto-decrypt
   if (typeof embeddedPublicPassword !== 'undefined' && embeddedPublicPassword !== 'none') {
     // Auto-decrypt with embedded password
     console.log('Auto-decrypting with embedded password...');
     setTimeout(() => {
       handleDecryptionAndLoading(embeddedPublicPassword).catch(err => {
         console.error('Auto-decryption failed:', err);
       });
     }, 500); // Small delay to ensure DOM is ready
   } else {
     // Create manual password input UI
     ...
     decryptButton.addEventListener('click', async () => {
       ...
       try {
         await handleDecryptionAndLoading(passphrase);
       } catch (err) {
  ...
  } // End of else block (phone check)
} // End of file/embedded mode block
