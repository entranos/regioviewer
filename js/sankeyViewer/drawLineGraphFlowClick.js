...
function drawBarGraph (data, config) {
  ...
  const barGroupWidth = graphWidth / years.length - 20 // Space for each year group
  const barWidth = barGroupWidth / varianten.length // Width per province bar

  // x-scale for years (categorical scale for bar groups)
  ...
  function updateGraph () {
    ...
    canvas.selectAll('g[class^="scenario-group-"]').remove()
    ...
    varianten.forEach((scenarioName) => {
      ...
      // Skip if no data exists for this scenario
      if (!scenarioDataForYears) {
        // console.warn(`No data found for scenario: ${scenarioName}`)
        return
      }

      // Use the flow/link color for all bars
      ...
      // Draw bars for this scenario across all years
      scenarioData.forEach((d, i) => {
        const year = d.year
        const yearX = xYear(year)
        const yearBandwidth = xYear.bandwidth()

        // Calculate how many visible scenarios have data for this year
        const visibleWithData = varianten.filter(s => {
          if (!globalVisibleScenarios.has(s)) return false
          const data = displayNameToDataMap[s]
          return data && data[year] !== undefined && data[year] !== null
        })

        const barWidth = yearBandwidth / visibleWithData.length
        const barIndex = visibleWithData.indexOf(scenarioName)
        const barX = yearX + barIndex * barWidth
        const barHeight = graphBottom - y(d.value)

        // Create a group for this bar
        const barGroup = canvas.append('g')
          .attr('class', `scenario-group-${scenarioName}-${year}`)
        ...
        // Add rotated province name label under the bar
        barGroup.append('text')
          .attr('class', 'bar-label')
          .attr('x', barX + (barWidth - 2) / 2)
          .attr('y', graphBottom + 20)
          .attr('transform', `rotate(-45, ${barX + (barWidth - 2) / 2}, ${graphBottom + 20})`)
          .style('text-anchor', 'end')
          .style('font-size', '10px')
          .style('fill', '#333')
          .style('pointer-events', 'none')
          .text(variantTitles[scenarioName] || scenarioName)
      })
    })
  }
  ...
  // Add year group labels and separators
  years.forEach((year, index) => {
    const yearX = xYear(year)
    const yearBandwidth = xYear.bandwidth()

    // Year label above the bars
    canvas.append('text')
      .attr('x', yearX + yearBandwidth / 2)
      .attr('y', graphTop - 10)
    ...
    // Add separator line between year groups (but not after the last one)
    if (index < years.length - 1) {
      const separatorX = yearX + yearBandwidth + (xYear.step() - yearBandwidth) / 2
      canvas.append('line')
        .attr('x1', separatorX)
        .attr('x2', separatorX)
        .attr('y1', graphTop)
  ...
  // x-axis (hidden year labels since we show them above)
  canvas.append('g')
    .attr('transform', `translate(0, ${graphBottom})`)
    .call(d3.axisBottom(xYear).tickFormat('').tickSize(0))
