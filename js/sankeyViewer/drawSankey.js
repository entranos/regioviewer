...
// Make currentUnit global and sync with map unit selector
window.currentUnit = window.globalDisplayUnits ? window.globalDisplayUnits.volume : 'PJ'
...
function drawSankey (sankeyDataInput, config) {
  ...
  // Note: .on() event handlers removed - not supported by d3-sankey-diagram
    // Hover functionality is implemented below using direct D3 selection
...
function drawUnitSelector () {
  d3.select('#unitSelector_sankey').append('div').attr('id', 'unitSelectorDiv').style('width', '200px').style('height', '35px').style('position', 'absolute').style('top', '0px').style('right', '0px').append('svg').attr('width', 200).attr('height', 35).attr('id', 'selectorButtonSVGSankey').attr('transform', 'scale(0.8)')
  ...
  sCanvas.append('rect')
    ...
    .style('pointer-events', 'auto')
    .on('click', function () {
      if (window.currentUnit == 'PJ') {window.currentUnit = 'TWh'} else window.currentUnit = 'PJ'
      d3.selectAll('#selectorStatus').transition().duration(200).attr('cx', function () {if (window.currentUnit == 'PJ') { return 63} else return 87})

      // Update global unit state (shared with map)
      if (window.globalDisplayUnits) {
        window.globalDisplayUnits.volume = window.currentUnit
      }
      ...
      // Update map unit selector toggle to match
      if (typeof updateMapUnitSelectorToggle === 'function') {
        updateMapUnitSelectorToggle()
      }

      // Update map visualization to show new units
      if (window.dataVisualizationState && window.dataVisualizationState.isActive) {
        if (typeof updateDataVisualization === 'function') {
  ...
  sCanvas.append('circle')
    ...
    .style('pointer-events', 'none')
    .attr('cx', function () {if (window.currentUnit == 'PJ') { return 63} else return 87})
    .attr('cy', 12.5)
    .attr('r', 10)
    .attr('fill', '#444')
  sCanvas.append('text')
